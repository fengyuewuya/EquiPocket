...
class Baseline_Models(nn.Module):
    def __init__(self, atom_channels, bond_channels, out_features, gat_depth=0, gcn_depth=0, gin_depth=0, gcn2_depth=0, schnet_depth=0, dimenet_depth=0, egnn_depth=0, cutoff=5):
        ...
        if self.egnn_depth > 0:
            ...
            self.egnn = EGNN(in_node_nf=out_features,
    ...
    def forward(self, batch_data):
        ...
        # schnet
        if self.schnet_depth > 0:
            node_embedding = self.schnet(z=batch_data.x[:, 0], pos=batch_data.pos, batch=batch_data.batch)
        ...
        if self.egnn_depth > 0:
            ...
            for i in batch_data.batch.unique():
                ...
                tmp_node_embedding, tmp_pos = self.egnn(h=tmp_node_embedding,
                        x=tmp_pos,
                        edges=edge_index,
                        edge_attr=edge_attr)
